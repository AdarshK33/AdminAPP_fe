...
const PendingReports = () => {
  ...
  const getPageNumber = (pageNumber) => {
    dispatch(listProjects({ pageNumber, status:"pending" }))
  }

  const getSearchKey = (searchKey) => {
    dispatch(listProjects({ searchKey, status:"pending" }))
  }
  ...
  useEffect(() => {
    if (deleteProjectSuccess !== null) {
      dispatch(listProjects({status:"pending"}))
  ...
  return (
    <>
      <ContentHeading>Pending Projects List</ContentHeading>
      <CustomDataTable title="Pending Projects" addButton="/add-project" data={data} updateRow={updateRow} deleteRow={deleteRow} getPageNumber={getPageNumber} getSearchKey={getSearchKey} />
